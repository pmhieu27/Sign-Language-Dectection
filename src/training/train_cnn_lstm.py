"""
Train the CNN1D + LSTM model with the current final-data pipeline.

Usage:
    python scripts/train.py cnn_lstm
    python scripts/train.py cnn_lstm --epochs 150 --seed 123
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train CNN1D + LSTM model")
...
def main():
    ...
    model = build_cnn_lstm_model(seq_len, num_features, num_classes, name="CNN1D_LSTM_SignLanguage_Final")
    model.compile(
    ...
    model_path = os.path.join(args.model_dir, "cnn_lstm_final.keras")
    ...
    report_path = os.path.join(args.model_dir, "cnn_lstm_final_test_report.json")
    ...
    print(
        f"Training CNN1D + LSTM model (epochs={args.epochs}, batch_size={args.batch_size}, "
    ...
    print("\n" + "=" * 60)
    print("CNN1D + LSTM TRAINING COMPLETE")
