...
def plot_confusion_matrix(cm, target_names, save_path, normalize=False):
    if normalize:
        ...
        fmt = ".1f"
        suffix = "%"
        title = "Confusion Matrix - Final Best Model (Normalized %)"
    else:
        cm_display = cm.astype(np.float64)
        fmt = ".0f"
        suffix = ""
        title = "Confusion Matrix - Final Best Model (Counts)"
...
def main():
    ...
    count_path = os.path.join(args.save_dir, "confusion_matrix_final_best.png")
    norm_path = os.path.join(args.save_dir, "confusion_matrix_final_best_normalized.png")
    ...
    report_path = os.path.join(args.save_dir, "evaluation_final_best_report.json")
    summary_path = os.path.join(args.save_dir, "evaluation_final_best_summary.json")
